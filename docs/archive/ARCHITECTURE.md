...
| **PostgreSQL** | `postgres:16` | 5432 | Gateway persistence sidecar (Podman pod only). Stores activity history, rule catalog, rule overrides, and ContentGraph snapshots on the `postgres-data` volume. Gateway connects via `APME_DATABASE_URL=postgresql+asyncpg://apme:apme@127.0.0.1:5432/apme`. Operators back up this volume (or use `pg_dump` against the sidecar) before upgrades. |
| **Gateway** | `apme-gateway` | 8080 / 50060 | Dual-protocol: FastAPI REST API (:8080) for the UI and a gRPC Reporting service (:50060) that receives `FixCompletedEvent` and `RegisterRules` from Engine. Persists activity history, rule catalog, rule overrides, and ContentGraph snapshots to PostgreSQL. Health endpoint probes all upstream services. REST endpoints include `/api/v1/rules` for rule catalog management (ADR-041) and `/api/v1/projects/{id}/graph` for ContentGraph visualization. |
...
| **postgres-data** | `/var/lib/postgresql/data` | PostgreSQL (rw) | Gateway persistence (Podman volume `apme-postgres-data`); back up before upgrades |
...
|------|---------|----------|
| 5432 | PostgreSQL | TCP (Gateway persistence sidecar) |
| 50051 | Engine | gRPC |
| 50053 | Ansible | gRPC |
