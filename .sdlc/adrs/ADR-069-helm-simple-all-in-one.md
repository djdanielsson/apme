...
- ADR-012 still defines the conceptual scaling unit (engine stack). The chart
  simply does **not** offer multi-replica engine scaling while Gateway PostgreSQL
...
3. **Single replica** — Chart defaults and validation: `replicas: 1`. HPA for
   ...
   that reintroduces a split (or otherwise solves Gateway PostgreSQL + session
   affinity).
...
- No independent engine HPA while Gateway PostgreSQL shares the pod.
