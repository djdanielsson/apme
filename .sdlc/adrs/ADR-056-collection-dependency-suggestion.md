# ADR-056: Collection Dependency Suggestion for Unresolved Modules (R501)
...
The mapping is a JSON file (`module_collection_map.json`) generated at image build time by querying a reference set of popular collections. R501 consults this mapping when L037 reports an unresolved module — if the short name appears in the map, R501 emits an advisory finding suggesting the collection.
...
R501 is an **advisory/suggestion** rule (severity INFO):
- Fires on PLAY or PLAYBOOK nodes (aggregate, not per-task)
- Checks unresolved short module names against the static map
- Emits a finding like: `Module 'nmcli' may be provided by collection 'community.general'. Consider adding it to requirements.yml.`
...
- Module name collisions across collections (rare but possible) require disambiguation logic
...
- [Issue #247](https://github.com/apme/apme/issues/247): Doc-only rules requiring implementation
